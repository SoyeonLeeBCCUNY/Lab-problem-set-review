...
This is an example of a reproducible report for part I of the PSYC 7709 semester project.
...
library(dplyr)
library(ggplot2)
all_data <- fread("open_data/stroop_stand.csv")
...
The re-analysis successfully reproduced the analysis reported by Rosenbaum, Mama, Algom (2017). One issue with this reproducible analysis document was that the results of the analysis were written by hand into to the document. In this case, errors could have been inserted by hand. It is possible to have R insert the results of an analysis directly into the text, and an example of this is given in the `APAreport.Rmd` example that uses `papaja` to write an APA paper.
